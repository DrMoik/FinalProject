...
class HoleDetector(Node):
    def __init__(self):
        ...
        self.source_frame_ = "D415_color_optical_frame"
    ...
    def image_callback(self, msg):
        ...
        detected_circles = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT_ALT, 1.5, 10, param1=300, param2=0.8, minRadius=10, maxRadius=15)
        detected_base = cv2.HoughCircles(blurred, cv2.HOUGH_GRADIENT_ALT, 1.5, 10, param1=300, param2=0.8, minRadius=50, maxRadius=100)
    ...
    def get_world_coord(self, image_coor):
        if self.cv_depth_ is None:
            self.get_logger().error('Aligned depth image not received')
            return []
        ...
        z = self.cv_depth_[image_coor[1], image_coor[0]] / 1000.0
        if z == 0:
            self.get_logger().error('Invalid depth')
            return []
