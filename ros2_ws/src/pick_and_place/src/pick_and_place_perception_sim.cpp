...
public:
  ...
  explicit GetPoseClient(
      ...
      : Node("get_pose_client", node_options) {
    // Callback group
    callback_group_ =
    ...
    // Create subscribers
    webpage_sub_ = this->create_subscription<std_msgs::msg::Int16>(
        "/webpage", 10,
        std::bind(&GetPoseClient::webpage_callback, this,
                  std::placeholders::_1),
        options1);
    // move it
    move_group_node_ =
    ...
    client_ptr_ = this->create_client<hole_detector::srv::HoleCoordinates>(
        "/holes_coordinates");
    ...
    timer_ = create_wall_timer(
        ...
        std::bind(&GetPoseClient::pick_and_place_service, this),
        callback_group_);
  }
...
private:
  ...
  void pick_and_place_callback(
      rclcpp::Client<hole_detector::srv::HoleCoordinates>::SharedFuture
          future_result) {
    // Get the response
    ...
    // Check for success
    if (!response->success)
      RCLCPP_ERROR(LOGGER, "Failed to get hole coordinates. Exiting.");

    RCLCPP_INFO(LOGGER, "Result received");
    RCLCPP_INFO(LOGGER, "X: %f, Y: %f, Z: %f", response->coordinates[2].x,
                response->coordinates[2].y, response->coordinates[2].z);
    ...
    // Compute a Cartesian path that follows specified waypoints with a step
    // size of at most eef_step meters between end effector configurations of
    // consecutive points in the result trajectory. The reference frame for the
    // waypoints is that specified by setPoseReferenceFrame(). No more than
    // jump_threshold is allowed as change in distance in the configuration
    // space of the robot (this is to prevent 'jumps' in IK solutions).
    // Collisions are avoided if avoid_collisions is set to true. If collisions
    // cannot be avoided, the function fails. Return a value that is between 0.0
    // and 1.0 indicating the fraction of the path achieved as described by the
    // waypoints. Return -1.0 in case of error.
    ...
    // We need to use a generic Constraints message, but this time we add it to
    // the orientation_constraints
    moveit_msgs::msg::Constraints orientation_constraints;
    orientation_constraints.orientation_constraints.emplace_back(
  ...
  void pick_and_place_service() {
    ...
    */
    if (go_) {
      // Reset flag
      go_ = false;
      RCLCPP_INFO(LOGGER, "TIMER_GO");

      // Call the service
      auto request =
          std::make_shared<hole_detector::srv::HoleCoordinates::Request>();
      client_ptr_->async_send_request(
          request, std::bind(&GetPoseClient::pick_and_place_callback, this,
                             std::placeholders::_1));
    }
  }
